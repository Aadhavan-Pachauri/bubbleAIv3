...
import { Project, Message, Plan, ProjectPlatform, Profile, Chat, ChatMode, Memory, ProjectType, MemoryLayer, AppSettings, ChatWithProjectData, Notification, Friendship } from '../types';
...
export const addMessage = async (supabase: SupabaseClient, message: Omit<Message, 'id' | 'created_at'>): Promise<Message> => {
    ...
    // SANITIZE: Remove properties that are for UI state only or don't exist in DB schema.
    delete (messageToInsert as any).imageStatus;
    delete (messageToInsert as any).groundingMetadata; // Prevent schema error if column missing
    delete (messageToInsert as any).isError;
...
export const getFriendships = async (supabase: SupabaseClient, userId: string): Promise<Friendship[]> => {
    ...
    return (data || []).map((f: any) => {
        ...
        return {
            id: f.id,
            user_id: userId,
            friend_id: isInitiator ? f.friend_id : f.user_id,
            status: f.status,
            created_at: f.created_at,
            other_user: isInitiator ? f.friend : f.initiator
        } as Friendship;
    });
};

export const getPendingFriendRequests = async (supabase: SupabaseClient, userId: string): Promise<any[]> => {
    // Requests where current user is the target (friend_id) and status is pending
    const { data, error } = await supabase
        .from('friendships')
        .select(`
            *,
...
export const updateFriendRequest = async (supabase: SupabaseClient, id: string, status: 'accepted' | 'blocked'): Promise<void> => {
    const { error } = await supabase
        .from('friendships')
        .update({ status })
        .eq('id', id);
